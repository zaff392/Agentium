...
const IDE_EXTENSION_PATHS = {
  ...
  cursor: {
    win32: [
      ...
      path.join(os.homedir(), '.cursor-insiders', 'extensions'),
    ],
    darwin: [
      path.join(os.homedir(), '.cursor', 'extensions'),
      path.join(os.homedir(), '.cursor-insiders', 'extensions'),
...
function detectIDEs() {
  ...
  // Trae
  const traePaths = {
    win32: [path.join(os.homedir(), 'AppData', 'Local', 'Programs', 'Trae', 'Trae.exe')],
    darwin: ['/Applications/Trae.app'],
    linux: ['/usr/bin/trae', path.join(os.homedir(), '.local', 'bin', 'trae')]
  };

  for (const traePath of (traePaths[platform] || [])) {
    if (fs.existsSync(traePath)) {
      ides.push({ name: 'Trae', path: traePath, type: 'trae' });
      break;
    }
  }
...
// Installation de l'extension pour l'IDE sélectionné
async function installExtensionForIDE(ideType) {
  const platform = os.platform();
  const idePaths = IDE_EXTENSION_PATHS[ideType];

  if (!idePaths) {
    logWarning(`No extension paths configured for IDE type: ${ideType}`);
    return false;
  }

  const extensionPaths = idePaths[platform] || idePaths.linux;

  let installed = false;

  for (const extPath of extensionPaths) {
    // We check if the parent directory (like .vscode or .cursor) exists, or if it's a typical extensions path
    if (fs.existsSync(path.dirname(extPath)) || extPath.includes('.' + ideType)) {
      try {
  ...
  // Essayer avec la commande CLI (code, cursor, trae)
  if (!installed) {
    try {
      const vsixPath = path.join(PACKAGE_ROOT, 'vscode-extension');
      if (fs.existsSync(vsixPath)) {
        let cliCommand = 'code';
        if (ideType === 'cursor') cliCommand = 'cursor';
        if (ideType === 'trae') cliCommand = 'trae';

        execSync(`${cliCommand} --install-extension "${vsixPath}" --force`, { stdio: 'pipe' });
        logSuccess(`Extension installed via ${ideType} CLI`);
        installed = true;
      }
    } catch (e) {
      logWarning(`Could not install via ${ideType} CLI`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    // Étape 5: Installation de l'extension
    logStep('5/6', 'Installing extension...');

    let targetIdes = ['vscode'];

    if (!options.skipPrompts) {
      console.log(`\n${colors.cyan}IDE Selection${colors.reset}`);
      console.log('Select the IDEs where you want to install Agentium:');
      console.log('1. VS Code');
      console.log('2. Cursor');
      console.log('3. Trae');
      console.log('4. All detected IDEs');

      const selection = await askQuestion(rl, 'Enter your choice (1-4) [default: 1]: ');
      ...
      if (selection === '2') {
        ...
        targetIdes = ides.map(ide => ide.type);
        // Ensure at least vscode is in the list if no IDE was detected
        if (targetIdes.length === 0) targetIdes = ['vscode', 'cursor', 'trae'];
      }
    } else if (options.ide) {
      if (options.ide === 'all') {
        targetIdes = ['vscode', 'cursor', 'trae'];
      } else {
    ...
    for (const ideType of targetIdes) {
      log(`Installing for ${ideType}...`, 'cyan');
      const installed = await installExtensionForIDE(ideType);
      if (installed) anyInstalled = true;
    }

    if (!anyInstalled) {
      logWarning('Extension could not be auto-installed to any selected IDE');
      logWarning('You can install it manually from the extensions folder');
    }
