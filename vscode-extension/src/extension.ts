...
<body>
  <div class="header">
    <h1>🤖 Agentium Chat</h1>
    <select class="agent-select" id="agentSelect" aria-label="Select an AI agent">
  ...
  <div class="messages" id="messages">
    <div class="empty-state" id="emptyState">
      ...
      <div class="quick-actions">
        <button onclick="selectAgent('react')" aria-label="Select React agent">⚛️ React</button>
        <button onclick="selectAgent('python')" aria-label="Select Python agent">🐍 Python</button>
        <button onclick="selectAgent('docker')" aria-label="Select Docker agent">🐳 Docker</button>
        <button onclick="selectAgent('sql')" aria-label="Select SQL agent">🗄️ SQL</button>
        <button onclick="selectAgent('security')" aria-label="Select Security agent">🛡️ Security</button>
      </div>
    </div>
  </div>
  ...
  <div class="input-area">
    <div class="input-wrapper">
      ...
      ></textarea>
      <button onclick="send()" id="sendBtn" aria-label="Send message">Send</button>
    </div>
  </div>
      ...
      processedContent = processedContent.replace(
        ...
        (match, lang, code) => {
          ...
          return '<pre><code class="language-' + (lang || 'text') + '">' + escapedCode + '</code>' +
            ...
            '<button onclick="copyCode(this)" aria-label="Copy code block">Copy</button>' +
            '<button onclick="insertCode(this)" aria-label="Insert code block at cursor">Insert</button>' +
            '</div></pre>';
        }
      );
